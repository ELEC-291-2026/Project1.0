...
WARNING_DELTA = 15.0   # °C – consider anything above this as "very large"
in_warning = False      # tracks whether we are currently in a warning state
...
def run(data):
    ...
    if t > -1:
        ...
        #large deviation warming========================
        global in_warning
        if abs(delta) > WARNING_DELTA:
            # Show warning on plot
            warn_text.set_text(f"WARNING: |Δ| > {WARNING_DELTA:.0f}°C (Δ={delta:+.2f}°C)")
            # Only print when entering the warning state, not every frame
            if not in_warning:
                print(f"[WARNING] Large temperature deviation: Δ={delta:+.2f}°C at t={t_seconds:.1f}s")
                in_warning = True
        else:
            # Clear warning when deviation returns to acceptable range
            if in_warning:
                print(f"[INFO] Deviation back to normal at t={t_seconds:.1f}s (Δ={delta:+.2f}°C)")
            in_warning = False
            warn_text.set_text("")
    #===================================================
    ...
    return segments, text, target_line, actual_line, warn_text
